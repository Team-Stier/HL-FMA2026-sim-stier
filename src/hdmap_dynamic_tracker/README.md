...
- 객체 점유: 현재 OBB와 `[min_z, min_z + height]`를 CellTree의 실제 polygon 교차로 계산한다.
  ...
  간격으로 다시 예측한 OBB들의 convex hull로 조회한다. 표본 사이 CTRA 곡선의 chord 이탈량도
  `max(|p''|)·dt²/8`로 추가 팽창해 endpoint 직선 보간에 의한 중간 누락을 방어한다. 위치 공분산은 설정한 sigma 배수를 hard cap 없이 모두
  적용한다. 최근 위치차에서 얻은 속도 벡터와 필터 상태가
  ...
  수용한 위치 innovation 뒤의 EKF 평활화 중심은 raw XY에 다시 고정해 이후 bin이 과거 궤적에
  남지 않게 한다. `heading`을 속도 방향으로 간주하지 않는다.
- 미래 객체의 body yaw는 관측 yaw rate로 별도 전파하며 L/W 반대각을 포함하는 외접 여유도 추가한다.
  미래 min-Z도 고정값으로 추정하지 않고 2D Cell 교차를 사용한다. 이로써 예측 중심 주위의 형상
  회전과 Z 변화로 인한 누락은 피하지만, 아직 관측되지 않은 미래 중심 궤적의 선회까지 결정론적으로
  보장하는 것은 아니다. 그 부분은 CTRA 공분산과 속도·방향 잔차가 정한 통계적 envelope에 의존한다.
